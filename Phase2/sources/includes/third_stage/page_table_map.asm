...
pushaq ;; defined as macro -> that pushes all 64 bit registers to stack / rax/rbx/rcx/r8-r15
  ;; we didn't use the rax pointer, to not mix it with other values, if it has a certain function number to exeucte
        ...
        ;inc [PML4_Counter] ;; limitation-> didnt work
        add QWORD[PML4_Counter], 0x1        ;increment PML4 counter 
        ...
        mov [r8], r9              ; ;; moving the address of the PD_pointer to the PDP_pointer VALUE
               ; inc QWORD[PDP_Counter]   ;limitation -> didnt work

        add QWORD[PDP_Counter], 0x1 ;;increment PDP counter 
       ...
       ; inc QWORD[PD_Counter]   ;limitation -> didnt work

        add QWORD[PD_Counter], 0x1;;increment PD counter
        ...
        ;; The start of the four loops
                ...
                _PML4_Loop:
                    ...
                    ; inc QWORD[PML4_Counter]   ;limitation -> didnt work
                    ...
                    add QWORD[PML4_Counter], 0x1    ;increment PML4 counter 
                            ...
                            _PDP_Loop:
                                ...
                                add QWORD[PDP_Counter], 0x1 ;increment PDP counter
                                        ...
                                        ;inc QWORD[PD_Counter];; limitation didnt work    
                                            add QWORD[PD_Counter], 0x1      ;increment PTE counter by 1
                                                ...
                                                _PTE_Loop:
                                                        ...
                                                        ;inc QWORD[PTE_Counter];; limitation didnt work    
                                                        add QWORD[PTE_Counter], 0x1         ;; increment the PTE counter by 1
...
_type_loop: ;; we have 4 types ; type 1 ,type 2, type 3, type 4 , the only one that is valid is type 1 otherwise we skip it
mov rax, QWORD[r14]  ; 1st region
mov rbx, QWORD[r14+8] ; increment to next region
add rax,rbx ; getting the addr
cmp rax,QWORD[Memory] ; checking if we reached the end if 
jle _which_region
add r14,0x18 ; increment to the next entry [24 bytes] of the table if above isn't satisfied 
jmp _type_loop ;; recursively

_which_region:
xor ecx,ecx ; make sure it is zero before any operation
mov ecx, DWORD[r14+16] ; increment register by 16 bytes
cmp ecx, 0x1 ; check if register is equal to 1 thenwe found the first region in memory and we need to store it to proceed
je _MemTypeRegion1
;;elsee
_MemTypeRegion2_3_4: ;; other types that cannot be mappes
add QWORD[Memory],0x1000 ;; increment by 1000 to point at the next one
jmp _memory ;; go back and repeat
_MemTypeRegion1:
jmp _continueMapping ;; if we are in type 1 we can map it so we will return to PTE to continue mapping
